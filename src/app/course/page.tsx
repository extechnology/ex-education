...
function Course() {
  return (
    <div className="pt-32 md:px-10 lg:px-0 bg-[#F4F7FB]">
      {Courses.map((course) => (
        <div key={course.id} className="max-w-7xl mx-auto pb-20 last:pb-7">
          ...
          <div className="pt-5">
            <h1 className="font-bold text-2xl text-transparent bg-clip-text bg-gradient-to-r from-fuchsia-600 via-50% to-blue-600 pt-3">
              {course.sub_title}
            </h1>
            <p className="text-justify font-medium p-5 bg-slate-200 rounded-2xl shadow-md mb-4 mt-2 ">
              {course.content}
            </p>
          </div>
          <div>
            {course.pros.map((pro,index) =>
              Object.entries(pro).map(([Key, values]) => (
                <div key={Key + index} className="py-10">
                  <h1 className="font-bold py-5 text-2xl text-center text-blue-900">{Key}</h1>
                  <ul className="grid grid-cols-5 gap-5 py-5">
                    {values.map((value,i) => (
                      <li key={i} className="p-3 shadow-md text-center content-center rounded-2xl backdrop-blur-2xl shadow-blue-300">
                        {value}
                      </li>
                    ))}
                  </ul>
